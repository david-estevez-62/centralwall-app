...
var localSignIn = new LocalStrategy({
  ...
  function(req, username, password, next){
    ...
    User.findOne({ "email": username }, function(err, user){
      ...
      if (!user){
        return next(null, false, { info: "No user found" });
      }
      ...
      user.comparePassword(password, function(err, isMatch){
        ...
        // Password did not match the returned user
        if(!isMatch){
          return next(null, false, { info: "Incorrect password." });
        }

        // user found, password match. Check if user has verified email
        if(!user.verified){
          return next(null, false, { info: "Confirm the email verification." });
        }else{
